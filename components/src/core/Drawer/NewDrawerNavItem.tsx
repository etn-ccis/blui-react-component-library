...
export type NewDrawerNavItem = SharedStyleProps &
    NavItemSharedStyleProps & {
        classes?: NewDrawerNavItemClasses;
        depth?: number;
        hidden?: boolean;
        hidePadding?: boolean;
        icon?: JSX.Element;
...
export type NewNestedDrawerNavItem = Omit<NewDrawerNavItem, 'icon'>;
...
const useStyles = makeStyles((theme: Theme) =>
    createStyles({
        ...
        infoListItem: {
            paddingLeft: (props: NewDrawerNavItem): number => calcNestedPadding(theme, props.depth),
        },
        listItemNoHover: {
            '&:hover': {
                /* TODO:
        ...
        nestedListGroup: {
            backgroundColor: (props: NewDrawerNavItem): string =>
...
const NewDrawerNavItemRender: React.ForwardRefRenderFunction<unknown, NewDrawerNavItem> = (
    props: NewDrawerNavItem,
    ref: any
) => {
            ...
            {items && items.length > 0 && (
                <Collapse in={expanded && drawerOpen !== false} key={`${itemTitle}_group_${depth}`}>
                    <List className={clsx(defaultClasses.nestedListGroup, classes.nestedListGroup)}>
                        {items.map((subItem: NewDrawerNavItem, index: number) => (
                            <NewDrawerNavItem
                                key={`itemList_${index}`}
                                {...subItem}
...
export const NewDrawerNavItem = React.forwardRef(NewDrawerNavItemRender);
NewDrawerNavItem.displayName = 'NewDrawerNavItem';
