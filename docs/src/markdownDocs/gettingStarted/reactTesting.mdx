...
React applications scaffolded with Create React App, or CRA, was the official way to start a new React project since its introduction in 2016. Unfortunately, CRA was deprecated in 2023. React applications scaffolded by create-react-app (CRA) came with Jest, a built-in unit testing framework.

The Brightlayer UI team researched modern ways to scaffold new React projects with the Brightlayer UI [React Component Library](https://github.com/etn-ccis/blui-react-component-library). Today and moving forward the Brightlayer UI
team adopts [Vite](https://v3.vitejs.dev/guide/why.html), a platform-agnostic frontend tool for building web applications. When using the Brightlayer UI [React Vite templates](https://github.com/etn-ccis/blui-react-cli-templates) to scaffold your new React project it will include [Vitest](https://vitest.dev/guide/why.html), the unit testing framework for React Vite projects.
...
They should cover user stories that span multiple components and views, such as signing up accounts, logging in and out of the application, updating a profile, etc. These tests are not concerned with the underlying state or implementation of the application - just the end results.
