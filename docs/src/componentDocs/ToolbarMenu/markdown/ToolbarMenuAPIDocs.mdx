...
The `menuGroups` prop of the `<ToolbarMenu>` includes many properties from the [`<DrawerNavGroup>`](/components/drawer-nav-group/api-docs) array found within a [`<DrawerBody>`](/components/drawer-body/api-docs).
