...
The `menuGroups` prop of the `<UserMenu>` includes many properties from the [`<DrawerNavGroup>`](/components/drawer/drawer-nav-group/api-docs) array found within a [`<DrawerBody>`](/components/drawer/drawer-body/api-docs).
