...
A `<Hero>` is used to call attention to a particular value or parameter. It adds some additional visual elements onto a [Channel Value](/components/channel-value/api-docs). When showing multiple heroes, they are typically placed within a Hero Banner to help with layout.
...
If you want to include an inline icon with the Channel Value, provide it via `ChannelValueProps` prop.
